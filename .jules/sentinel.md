...
## 2026-05-25 - [Bypassing Regex Checks with Leading Spaces]
**Vulnerability:** The external links plugin (`_plugins/external_links.rb`) used regex `/<a[^>]+href\s*=\s*['"]?(?:https?:|\/\/)/i` to detect if external links existed. However, if a link had leading spaces in the `href` attribute (e.g., `href=" https://google.com"`), the regex would fail to match it, meaning `rel="noopener noreferrer"` was not added, leaving the application vulnerable to window.opener exploits.
**Learning:** Whitespace inside HTML attributes is technically invalid but heavily tolerated by all major browsers. Attackers can leverage browser tolerance to bypass regex-based security filters if the regexes are overly rigid and don't account for whitespace within the attribute value itself.
**Prevention:** Whenever matching HTML attributes, ensure regex checks allow for optional leading and trailing whitespace around values `\s*` inside quotes, and strip values (`.strip`) before performing exact matching logic.
