...
## 2026-06-03 - [DOM Injection via innerHTML]
**Vulnerability:** The `assets/js/anchor-links.js` file used `anchor.innerHTML = '#'` to inject a text character into a newly created anchor button. While currently injecting a static string, using `innerHTML` for plain text or characters is a known anti-pattern that can lead to DOM-based XSS if the input is ever changed to be user-controllable.
**Learning:** Using `innerHTML` is inherently dangerous because it parses the input as HTML, potentially executing embedded scripts. When injecting plain text, characters, or variables that do not require HTML parsing, a safer DOM sink should be used.
**Prevention:** Always use `textContent` instead of `innerHTML` when the intended content is plain text. This ensures the browser treats the input strictly as text, preventing any malicious HTML or script execution.
