## 2026-01-05 - Enhanced Clickjacking Protection
**Vulnerability:** Standard frame-busting (`top.location = self.location`) fails silently inside `sandbox="allow-scripts"` iframes, leaving content visible and vulnerable.
**Learning:** JS-based frame busting must fail securely by hiding content (`display: none`) *before* attempting navigation. This ensures protection even if navigation is blocked.
**Prevention:** Updated `security.js` to hide `document.documentElement` immediately upon detecting a frame.
