...
## 2025-02-21 - [Reverse Tabnabbing Bypass]
**Vulnerability:** External link protection (Reverse Tabnabbing prevention) in `_plugins/external_links.rb` could be bypassed using case variations like `target="_BLANK"` or by injecting leading whitespace in the `href` attribute.
**Learning:** CSS selectors in Nokogiri like `a[target="_blank"]` are case-sensitive by default, missing `_BLANK`. Also, regex checks like `\A(https?:|//)` fail if the `href` contains leading spaces (which browsers tolerate).
**Prevention:** Use case-insensitive selectors (e.g., XPath `translate()` or CSS `[target="_blank" i]`) when searching for attributes. Always `.strip` extracted attributes like `href` before applying regex validations. Ensure early-return regex optimizations account for whitespace.
